...
const char *ADMIN_USER = "superior"; // change!
const char *ADMIN_PASS = "sup@123";  // change!

const char *BRAND_NAME = "INTELLIGENT LIGHTING SYSTEM"; // your personal brand
const char *TEAM_NAME = "THE SUPERIORS";                // your team name
...
String htmlHeader(const String &title)
{
    ...
    h += "<title>" + title + "</title>";
    h += "<style>";
    h += "body{font-family:Arial;text-align:center;background:#f4f4f4;padding:20px;}";
    h += "h1{margin-bottom:10px;}";
    h += ".btn{padding:12px 20px;font-size:18px;margin:8px;border:none;border-radius:8px;cursor:pointer;}";
    h += ".on{background:green;color:white;}.off{background:red;color:white;}";
    h += ".logout{background:#555;color:white;font-size:14px;padding:6px 12px;margin-top:20px;}";
    h += "form{display:inline-block;background:white;padding:20px;border-radius:10px;box-shadow:0 0 10px rgba(0,0,0,.1);}";
    ...
    h += "input[type=submit]{width:220px;margin-top:10px;}";
    h += "table{margin:0 auto;margin-top:20px;border-collapse:collapse;font-size:20px;}";
    h += "td{padding:10px 15px;}";
    h += "</style></head><body>";
    return h;
}
...
String loginPage(const String &msg = "")
{
    ...
    html += "<input class='btn on' type='submit' value='Login'>";
    html += "</form>";
    html += htmlFooter();
    return html;
}
